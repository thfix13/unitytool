...
namespace EditorArea {
	...
	public class ClusteringEditor : Editor {
		...
		public void OnSceneGUI () {			
			...
			// Create grid
			if (grid == null || grid.Length != ClusteringEditorWindow.gridSize) {
				grid = new Cell[ClusteringEditorWindow.gridSize][];
				for (int i = 0; i < ClusteringEditorWindow.gridSize; i++)
					grid [i] = new Cell[ClusteringEditorWindow.gridSize];
			}
			...
			if (drawer == null) {
				drawer = mapper.gameObject.GetComponent<ClusteringEditorDrawer> ();
			}
			...
			// Update Scene
			if (!didUpdate) {
				mapper.ComputeTileSize (SpaceState.Editor, mapper.collider.bounds.min, mapper.collider.bounds.max, ClusteringEditorWindow.gridSize, ClusteringEditorWindow.gridSize);
