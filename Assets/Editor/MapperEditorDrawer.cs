...
public class MapperEditorDrawer : MonoBehaviour
{
	...
	public List<GraphNode> graphNodesList = null;
	...
	public void OnDrawGizmos ()
	{
		...
		if (drawVoronoiForEnemies && eVoronoiGrid != null) {
			for (int x = 0; x < eVoronoiGrid.Length; x++) {
				for (int y = 0; y < eVoronoiGrid[x].Length; y++) {
					Cell c = eVoronoiGrid [x] [y];
					if (!c.blocked) {
		...
		if (drawVoronoiForCameras && cVoronoiGrid != null) {
			for (int x = 0; x < cVoronoiGrid.Length; x++) {
				for (int y = 0; y < cVoronoiGrid[x].Length; y++) {
					Cell c = cVoronoiGrid [x] [y];
					if (!c.blocked) {
						switch (c.nearestVoronoiCentre) {
						case 0:
							Gizmos.color = Color.red;
		...
		if (drawGraph && sBoundaryGrid != null && graphNodesList.Count != 0) {
