...
//	was here
	...
	public Vector3 movePlayer(float t){
		if(t<v[path[currentpath_i]].y)
			return Vector3.zero;
		while(t>v[path[currentpath_i+1]].y)
			currentpath_i++;
		if(currentpath_i>path.Count)
			return Vector3.zero;
		float frac1=t- v[path[currentpath_i]].y, frac2= v[path[currentpath_i+1]].y -t;
		var playerpos= (v[path[currentpath_i]]*frac2 + v[path[currentpath_i+1]] *frac1)/(frac1+frac2);
		return playerpos;
	}
