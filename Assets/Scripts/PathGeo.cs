...
namespace Common {
	public class PathGeo {
		...
		public PathGeo (List<NodeGeo> points) {
			this.points = points;
			if (points == null)
	...
	public class PathGeoBulk {
		public List<PathGeo> paths;
		
		public PathGeoBulk () {
			paths = new List<PathGeo> ();
		}
		
		public static void SavePathsToFile (string file, List<PathGeo> paths) {
			XmlSerializer ser = new XmlSerializer (typeof(PathBulk));
			
			PathGeoBulk bulk = new PathGeoBulk ();
		...
		public static List<PathGeo> LoadPathsFromFile (string file) {
			XmlSerializer ser = new XmlSerializer (typeof(PathGeoBulk));
			
			PathGeoBulk loaded = null;
			using (FileStream stream = new FileStream (file, FileMode.Open)) {
				loaded = (PathGeoBulk)ser.Deserialize (stream);
				stream.Close ();
			}
			
			// Setup parenting
			foreach (PathGeo p in loaded.paths) {
