...
namespace RRTController {
	...
	public class LineOfSight : Controller {
		...
		public LineOfSight (float limit) {
			path = new Path();
			paths = new List<Path>();
		...
		public bool validateLineOfSight (Node from, Node to, Node hit, RRTKDTreeCombat context) {
			...
			path.ZeroValues();
			Analyzer.ComputePathsLoSValues(paths, context.enemies, context.min, context.tileSizeX, context.tileSizeZ, context.nodeMatrix);
